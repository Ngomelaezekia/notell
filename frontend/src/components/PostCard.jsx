    ...
    <>
      <article className="overflow-hidden rounded-2xl border border-neutral-900 bg-neutral-950/70 shadow-[0_8px_30px_rgba(0,0,0,0.12)] transition-[border-color,box-shadow] duration-200 hover:border-neutral-800 hover:shadow-[0_10px_34px_rgba(0,0,0,0.16)] sm:rounded-[1.35rem]">
        ...
        <section className="px-3 pb-3 sm:px-4 sm:pb-4">
          ...
          {caption && <div className="mt-2.5 text-sm leading-6 text-neutral-200 sm:mt-3"><p className="whitespace-pre-wrap break-words"><button type="button" onClick={openAuthorProfile} disabled={!authorId} className="mr-2 font-semibold text-neutral-100 hover:underline disabled:cursor-default">{username}</button>{visibleCaption}</p>{hasLongCaption && <button type="button" onClick={() => setShowFullCaption((previous) => !previous)} className="mt-1 text-xs font-semibold text-neutral-500 transition hover:text-neutral-200">{showFullCaption ? "Show less" : "Read more"}</button>}</div>}
          <div className={`overflow-hidden transition-[grid-template-rows,opacity] duration-300 ${showComments ? "grid grid-rows-[1fr] opacity-100" : "grid grid-rows-[0fr] opacity-0"}`}><div className="min-h-0">{showComments && <CommentSection postId={postId} onCommentCountChange={setCommentCount} />}</div></div>
        </section>
      </article>

      {lightboxOpen && !isVideo && <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/95 p-3 backdrop-blur-sm sm:p-6" role="dialog" aria-modal="true" aria-label="Image viewer" onClick={() => setLightboxOpen(false)}><button type="button" onClick={() => setLightboxOpen(false)} className="absolute right-3 top-3 z-10 flex h-10 w-10 items-center justify-center rounded-full bg-white/10 text-white transition hover:bg-white/20 active:scale-90 sm:right-5 sm:top-5" aria-label="Close image viewer"><X size={21} /></button><img src={mediaUrl} alt={caption || "Post"} className="max-h-[92dvh] max-w-full object-contain" onClick={(event) => event.stopPropagation()} /></div>}
