...
function Stat({ label, value, icon: Icon, onClick }) {
  return (
    ...
    >
      ...
      <span>
        <span className="block text-base font-bold leading-5 text-white">
          {value ?? 0}
        </span>
        <span className="block text-[10px] font-medium uppercase tracking-wide text-white/55 group-hover:text-white/75">
          {label}
        </span>
      </span>
    </button>
...
function MediaTile({ post }) {
  ...
  return (
    <Link
      ...
      aria-label={`Open ${isVideo ? "video" : "photo"}${post?.caption ? `: ${post.caption}` : ""}`}
      className="group relative aspect-square overflow-hidden rounded-xl bg-slate-900 ring-1 ring-white/10 transition duration-200 hover:-translate-y-0.5 hover:ring-orange-400/35 focus:outline-none focus-visible:ring-2 focus-visible:ring-orange-400/80 active:scale-[0.99] sm:rounded-2xl"
    >
      {isVideo ? (
        <video
          src={mediaUrl}
      ...
      {isVideo && (
        <span className="absolute right-2 top-2 inline-flex h-7 w-7 items-center justify-center rounded-full bg-black/65 text-white shadow-lg backdrop-blur-sm">
          <Play size={12} fill="currentColor" />
        </span>
      )}
      {post?.caption && (
        <span className="absolute bottom-2 left-2 right-2 line-clamp-2 text-[10px] font-medium leading-4 text-white opacity-0 transition-opacity duration-200 group-hover:opacity-100">
          {post.caption}
        </span>
      )}
    </Link>
        ...
        {loading ? (
          ...
          <div className="overflow-y-auto">
            ...
            <div className="divide-y divide-slate-100">
              {users.map((person) => {
                const personAvatar = getFileUrl(person.profilePicture);
                return (
                  <Link
                    key={person.id}
                    to={`/users/${person.id}`}
...
function ProfileMenu({ onClose, onReport }) {
    ...
    try {
      if (navigator.share) {
        await navigator.share({
          ...
          text: "Check out this Notell profile",
          url,
        });
      } else {
  ...
  return (
    <>
      ...
      />
      <div
        className="absolute right-3 top-12 z-50 w-56 overflow-hidden rounded-2xl border border-white/10 bg-slate-900/95 p-1.5 shadow-2xl shadow-black/50 backdrop-blur-xl"
        role="menu"
      >
        <button
          type="button"
          ...
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold text-white/80 transition hover:bg-white/10 hover:text-white"
        >
          <Share2 size={15} />
        ...
        <button
          ...
          role="menuitem"
          onClick={onReport}
          className="flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-xs font-semibold text-red-300/90 transition hover:bg-red-500/10 hover:text-red-200"
        >
          <Flag size={15} />
...
function ReportDialog({ username, onClose }) {
  return (
    <div className="fixed inset-0 z-[60] flex items-end justify-center bg-black/70 p-0 backdrop-blur-sm sm:items-center sm:p-5">
      <section
        className="w-full rounded-t-3xl border border-white/10 bg-slate-900 p-5 shadow-2xl sm:max-w-md sm:rounded-3xl"
        role="dialog"
        aria-modal="true"
      ...
      >
        ...
        <div className="flex items-start gap-3">
          ...
          <div className="min-w-0">
            <h2 id="report-profile-title" className="text-base font-bold text-white">
              Report profile?
            </h2>
            <p className="mt-1 text-xs leading-5 text-white/55">
              You’re about to report <span className="font-semibold text-white/80">@{username}</span>.
              Reporting will be connected to moderation once the server endpoint is available.
            </p>
          </div>
        </div>
        <div className="mt-5 flex gap-2">
          <button
            type="button"
            onClick={onClose}
            className="min-h-11 flex-1 rounded-xl border border-white/10 bg-white/[0.05] px-4 text-xs font-bold text-white/75 transition hover:bg-white/10"
          >
            Cancel
          </button>
          <button
            type="button"
            onClick={onClose}
            className="min-h-11 flex-1 rounded-xl bg-red-500/15 px-4 text-xs font-bold text-red-300 ring-1 ring-red-400/20 transition hover:bg-red-500/25"
          >
            Report
          </button>
        </div>
...
export const Users = () => {
  ...
  const openRelationshipPanel = async (type) => {
    ...
    try {
      ...
      setRelationshipError(
        err.response?.data?.message || `Failed to load ${type}`,
      );
    } finally {
  ...
  const handleFollow = async () => {
    if (
      !id ||
      actionLoading ||
      (!relationship?.following && !relationship?.allowFollowers)
    ) {
      return;
    }
  ...
  if (error && !user) {
    return (
      <div className="mx-auto max-w-xl p-6">
        <Link to="/" className="mb-5 inline-flex items-center gap-2 text-sm font-medium text-white/70">
          <ArrowLeft size={17} />
          Back to home
        </Link>
        <div className="rounded-3xl border border-red-500/20 bg-slate-900 p-8 text-center text-red-300 shadow-sm">
          {error}
        </div>
      </div>
    );
      ...
      <div className="mx-auto w-full max-w-6xl">
          ...
          <div className="relative mx-auto max-w-4xl px-4 pb-5 sm:px-6 lg:px-8">
            <div className="-mt-14 flex flex-col items-center sm:-mt-16 lg:-mt-20">
              ...
              <div className="mt-3 text-center">
                ...
                {user?.bio && (
                  <p className="mx-auto mt-3 max-w-xl whitespace-pre-line text-sm leading-5 text-white/70">
                    {user.bio}
                  </p>
                )}
                  ...
                  <div className="mt-3 flex max-w-xl flex-wrap justify-center gap-1.5">
                    {bioLinks.map((url) => (
                      <a
                        ...
                        className="inline-flex min-h-9 max-w-full items-center gap-1.5 rounded-full border border-white/10 bg-white/[0.06] px-3 py-1.5 text-[10px] font-semibold text-white/80 transition hover:border-orange-400/40 hover:bg-orange-500/10 hover:text-white"
                      >
                        <Link2 size={12} className="shrink-0 text-orange-300" />
                        <span className="max-w-[9rem] truncate">{getLinkLabel(url)}</span>
                        <ExternalLink size={10} className="shrink-0 text-white/45" />
                      </a>
                    ))}
                  </div>
                )}
              ...
              {!isSelf && (
                <div className="mt-5 flex w-full max-w-lg items-stretch gap-2 sm:gap-3">
                  <button
                    ...
                    className={`group relative inline-flex min-h-12 flex-[1.5] items-center justify-center gap-2.5 rounded-2xl px-5 text-sm font-bold shadow-lg transition-all active:scale-[0.98] disabled:cursor-not-allowed disabled:opacity-50 sm:min-h-14 sm:text-[15px] ${relationship?.following ? "border border-white/20 bg-white/10 text-white shadow-black/20 hover:bg-white/15" : "bg-orange-500 text-white shadow-orange-950/30 hover:bg-orange-400"}`}
                  >
                    {actionLoading ? (
                  ...
                  <button
                    ...
                    className="inline-flex min-h-12 w-12 shrink-0 items-center justify-center rounded-2xl border border-orange-500/60 bg-orange-500/5 text-orange-300 transition hover:bg-orange-500/10 sm:min-h-14 sm:w-14"
                  >
            ...
            <div className="mx-auto mt-5 flex w-full max-w-xl items-center rounded-2xl border border-white/10 bg-white/[0.04] p-1">
              <Stat label="Posts" value={posts.length} icon={Camera} />
              <div className="h-8 w-px bg-white/10" />
              <Stat
                label="Followers"
                value={relationship?.followerCount}
                icon={UsersIcon}
                onClick={() => void openRelationshipPanel("followers")}
              />
              <div className="h-8 w-px bg-white/10" />
              <Stat
                label="Following"
                value={relationship?.followingCount}
                icon={UserRound}
                onClick={() => void openRelationshipPanel("following")}
              />
            </div>
        ...
        <section className="mx-auto mt-3 max-w-6xl px-3 sm:px-6">
          <div className="overflow-hidden rounded-3xl border border-white/10 bg-white/[0.035] shadow-2xl shadow-black/20">
            <div className="flex items-center justify-between border-b border-white/10 px-3 py-3 sm:px-4">
              <div>
                <div className="flex items-center gap-2">
                  <h2 className="text-sm font-bold text-white">Posts</h2>
                  {posts.length > 0 && (
                ...
                <p className="mt-0.5 text-[10px] text-white/40">
                  Photos and videos from {user?.username}
                </p>
              </div>
              <button
                type="button"
                aria-label="Post view options"
                className="inline-flex h-8 w-8 items-center justify-center rounded-full text-white/45 transition hover:bg-white/10 hover:text-white"
              >
            ...
            {posts.length === 0 ? (
              <div className="flex min-h-64 flex-col items-center justify-center px-6 py-12 text-center sm:min-h-72">
                <div className="relative flex h-16 w-16 items-center justify-center rounded-2xl border border-white/10 bg-white/[0.05] text-white/35 shadow-inner">
                  <Camera size={25} />
                  <span className="absolute -right-1.5 -top-1.5 flex h-6 w-6 items-center justify-center rounded-full border border-slate-950 bg-orange-500 text-white">
                ...
                <p className="mt-1 max-w-xs text-xs leading-5 text-white/40">
                  When {user?.username} shares photos or videos, they’ll appear here.
                </p>
                {isSelf && (
                  <Link
                    to="/create-post"
                    className="mt-5 inline-flex min-h-10 items-center gap-2 rounded-xl bg-orange-500 px-4 text-xs font-bold text-white shadow-lg shadow-orange-950/30 transition hover:bg-orange-400 active:scale-[0.98]"
                  >
                    <Camera size={14} />
              ...
              <div className="grid grid-cols-2 gap-1.5 p-2 sm:grid-cols-3 sm:gap-2.5 sm:p-3 lg:grid-cols-4 lg:gap-3">
                {posts.map((post) => (
