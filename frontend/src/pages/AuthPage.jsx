...
export default function AuthPage() {
  ...
  const switchMode = () => { setIsLogin((prev) => !prev); setLocalError(""); setFormData(initialForm); };
  ...
  return (
    <main className="relative min-h-[100dvh] w-full flex items-center justify-center overflow-y-auto p-4">
        ...
        <div className="rounded-[32px] border border-white/30 bg-white/10 backdrop-blur-xl shadow-2xl p-6 text-white">
          ...
          <AnimatePresence mode="wait">
            <motion.form key={isLogin} initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -15 }} transition={{ duration: 0.2 }} onSubmit={handleSubmit} className="space-y-3">
              {!isLogin && <Input icon={User} name="username" placeholder="Username" value={formData.username} onChange={handleChange} required />}
              <Input icon={Mail} name="email" type="email" placeholder="Email address" value={formData.email} onChange={handleChange} required />
              {!isLogin && <div className="grid grid-cols-2 gap-3"><Input icon={Globe} name="country" placeholder="Country" value={formData.country} onChange={handleChange} /><Input icon={MapPin} name="city" placeholder="City" value={formData.city} onChange={handleChange} /></div>}
...
function Input({ icon: Icon, ...props }) {
  return <div className="relative"><input {...props} className="w-full h-11 rounded-xl bg-white/10 border border-white/30 px-4 pr-11 text-sm placeholder:text-white/60 outline-none focus:border-white" /><Icon className="absolute right-4 top-1/2 -translate-y-1/2 text-white/60" size={16} /></div>;
}
