...
import { Search, UserRound, FileText, Loader2 } from "lucide-react";
...
export default function SearchPage() {
  ...
  useEffect(() => {
    ...
    setQuery(value);
    setActiveTab(TABS.some((tab) => tab.id === type) ? type : "all");
    ...
    Promise.allSettled([userAPI.searchUsers(value), postsAPI.searchPosts(value)])
      .then(([userResult, postResult]) => {
        if (cancelled) return;

        if (userResult.status === "fulfilled") {
          setUsers(userResult.value?.data?.users || []);
        } else {
          setUsers([]);
        ...
        if (postResult.status === "fulfilled") {
          setPosts(postResult.value?.data?.posts || []);
        } else {
          setPosts([]);
  ...
  const hasErrors = (showPeople && userError) || (showPosts && postError);
        ...
        <div className="mt-5 flex gap-2 overflow-x-auto border-b border-slate-100 pb-3">
          {TABS.map((tab) => (
            <button
              ...
              className={`shrink-0 rounded-xl px-4 py-2 text-sm font-bold transition ${activeTab === tab.id ? "bg-slate-900 text-white" : "bg-slate-100 text-slate-600 hover:bg-slate-200"}`}
            >
              {tab.label}
            </button>
          ))}
