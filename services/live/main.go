...
type Stream struct {
    ...
    PlaybackToken string `gorm:"size:128;uniqueIndex" json:"-"`
...
func auth(secret string) gin.HandlerFunc { return func(c *gin.Context) {
    token:=c.GetHeader("Authorization");if strings.HasPrefix(token,"Bearer "){token=strings.TrimSpace(strings.TrimPrefix(token,"Bearer "))}else{token,_=c.Cookie("auth_token")};if token==""{c.AbortWithStatusJSON(401,gin.H{"message":"authentication required"});return}
    parsed,err:=jwt.Parse(token,func(t *jwt.Token)(interface{},error){if t.Method.Alg()!=jwt.SigningMethodHS256.Alg(){return nil,errors.New("invalid signing method")};return []byte(secret),nil},jwt.WithIssuer("notell-api"));if err!=nil||!parsed.Valid{c.AbortWithStatusJSON(401,gin.H{"message":"invalid authentication"});return}
    sub,err:=parsed.Claims.GetSubject();if err!=nil||sub==""{c.AbortWithStatusJSON(401,gin.H{"message":"invalid authentication"});return};id,err:=strconv.ParseUint(sub,10,64);if err!=nil||id==0{c.AbortWithStatusJSON(401,gin.H{"message":"invalid authentication"});return};c.Set("userId",uint(id));c.Next()
} }
func randomKey() string { b:=make([]byte,24);if _,err:=rand.Read(b);err!=nil{panic(err)};return hex.EncodeToString(b) }
func playbackToken(secret string, streamID uint, ttl time.Duration) (string,error) { now:=time.Now().UTC();return jwt.NewWithClaims(jwt.SigningMethodHS256,jwt.MapClaims{"streamId":streamID,"scope":"live:play","iat":now.Unix(),"exp":now.Add(ttl).Unix(),"iss":"notell-live"}).SignedString([]byte(secret)) }
func main(){
    dsn:=os.Getenv("DATABASE_URL");secret:=os.Getenv("JWT_SECRET");if dsn==""||secret==""{panic("DATABASE_URL and JWT_SECRET are required")};db,err:=gorm.Open(postgres.Open(dsn),&gorm.Config{});if err!=nil{panic(err)};if err:=db.AutoMigrate(&Stream{});err!=nil{panic(err)}
    ...
    api.POST("/streams/:id/end",func(c *gin.Context){uid,_:=userID(c);var s Stream;if db.First(&s,c.Param("id")).Error!=nil{c.JSON(404,gin.H{"message":"stream not found"});return};if s.OwnerID!=uid{c.JSON(403,gin.H{"message":"stream owner required"});return};if s.Status!=Live{c.JSON(409,gin.H{"message":"stream is not live"});return};now:=time.Now().UTC();s.Status=Ended;s.EndedAt=&now;s.PlaybackToken="";if db.Save(&s).Error!=nil{c.JSON(500,gin.H{"message":"failed ending stream"});return};c.JSON(200,gin.H{"stream":s})})
    ...
    api.POST("/streams/:id/playback-token",func(c *gin.Context){var s Stream;if db.First(&s,c.Param("id")).Error!=nil||s.Status!=Live{c.JSON(404,gin.H{"message":"live stream not found"});return};token,e:=playbackToken(secret,s.ID,5*time.Minute);if e!=nil{c.JSON(500,gin.H{"message":"failed generating playback token"});return};c.JSON(200,gin.H{"token":token,"expiresAt":time.Now().UTC().Add(5*time.Minute)})})
    port:=os.Getenv("PORT");if port==""{port="8080"};if err:=http.ListenAndServe(":"+port,r);err!=nil{panic(err)}
}
