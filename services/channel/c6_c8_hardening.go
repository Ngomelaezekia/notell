...
func hardenedStartLive(db *gorm.DB)gin.HandlerFunc{return func(c *gin.Context){c.JSON(409,gin.H{"message":"live transport lifecycle is owned by the live service","code":"LIVE_LIFECYCLE_OWNED_BY_LIVE_SERVICE"})}}
func hardenedEndLive(db *gorm.DB)gin.HandlerFunc{return func(c *gin.Context){c.JSON(409,gin.H{"message":"live transport lifecycle is owned by the live service","code":"LIVE_LIFECYCLE_OWNED_BY_LIVE_SERVICE"})}}
func hardenedCancelLive(db *gorm.DB)gin.HandlerFunc{return func(c *gin.Context){ch,ok:=hardenedChannelPermission(db,c,livePermission());if !ok{return};var x LiveSession;if db.Where("id=? AND channel_id=?",c.Param("sessionId"),ch.ID).First(&x).Error!=nil{c.JSON(404,gin.H{"message":"live session not found"});return};if x.State!=LiveScheduled{c.JSON(409,gin.H{"message":"only scheduled sessions can be cancelled"});return};x.State=LiveCancelled;if err:=db.Save(&x).Error;err!=nil{c.JSON(500,gin.H{"message":"failed to cancel live schedule"});return};c.JSON(200,x)}}
