...
func usageStatusAllowed(status string) bool { switch status { case "incomplete", "active", "past_due", "paused": return true }; return false }
...
func registerPackageRoutes(r *gin.Engine, s *Server) {
 ...
 api.POST("/packages/:id/subscribe",func(c *gin.Context){ uid:=c.GetString("user_id"); var p Package; if err:=s.db.Table("packages").Where("id = ? AND active = true",strings.TrimSpace(c.Param("id"))).First(&p).Error; err!=nil { c.JSON(404,gin.H{"error":"package not found"}); return }; now:=time.Now().UTC(); sub:=Subscription{ID:newID("sub"),UserID:uid,PackageID:p.ID,ResourceType:"platform",ResourceID:p.ID,PriceID:p.ID,Provider:"stripe",Status:"incomplete",CurrentPeriodStart:now,CurrentPeriodEnd:now.AddDate(0,1,0)}; if err:=s.db.Create(&sub).Error; err!=nil { c.JSON(409,gin.H{"error":"subscription could not be created"}); return }; c.JSON(http.StatusCreated,gin.H{"subscription":sub,"payment_required":p.MonthlyPrice>0}) })
 api.POST("/usage",func(c *gin.Context){
  ...
  txErr:=s.db.Transaction(func(tx *gorm.DB) error {
   ...
   if err:=tx.Where("id = ? AND user_id = ?",subID,uid).Clauses(clause.Locking{Strength:"UPDATE"}).First(&sub).Error; err!=nil { return errors.New("subscription not found") }
   ...
   c.Set("created_usage",ev); return nil
  })
  ...
  ev:=c.MustGet("created_usage").(UsageEvent); summaries,err:=s.calculateCurrentUsage(uid,subID); if err!=nil { c.JSON(http.StatusCreated,gin.H{"usage":ev}); return }; c.JSON(http.StatusCreated,gin.H{"usage":ev,"billing":summaries})
 })
}
