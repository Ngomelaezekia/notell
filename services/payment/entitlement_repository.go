...
func upsertEntitlement(db *gorm.DB, event EntitlementEvent, planID string) error {
	...
	if err == gorm.ErrRecordNotFound {
		return db.Create(&Entitlement{
			ID: newID("ent"), UserID: event.UserID, ResourceType: event.ResourceType, ResourceID: event.ResourceID,
			PlanID: planID, Status: event.Status, StartsAt: event.EffectiveAt, EndsAt: event.EndsAt,
		}).Error
	}
	if err != nil {
		return err
