...
func validateCostRequest(in *CostEstimateRequest) (string, bool) {
 if in.StreamMinutes < 0 || in.ViewerCount < 0 || in.ViewerMinutes < 0 || in.TranscodeMinutes < 0 || in.StorageGBMonths < 0 || in.ComputeMinutes < 0 || in.CDNMinutes < 0 || in.PaymentFeeMicros < 0 || in.OperationalMicros < 0 { return "cost inputs cannot be negative", false }
...
func registerCostRoutes(r *gin.Engine, s *Server) {
 ...
 api.POST("/cost/estimate",func(c *gin.Context){
  ...
  breakdown,ok:=CalculateCost(costInput,s.cfg.CostRates); if !ok { c.JSON(http.StatusBadRequest,gin.H{"error":"cost calculation overflow"}); return }
 ...
 api.GET("/subscriptions/:id/cost",func(c *gin.Context){
  ...
  packagePriceMicros,ok:=mulSafe(p.MonthlyPrice,10000); if !ok { c.JSON(http.StatusInternalServerError,gin.H{"error":"package price overflow"}); return }
  packageCharge,ok:=addSafe(packagePriceMicros,overage); if !ok { c.JSON(http.StatusInternalServerError,gin.H{"error":"package charge overflow"}); return }
