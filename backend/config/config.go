...
func (c *Config) Validate() error {
	...
	if appEnv == "production" {
		...
		if err := validateHTTPSURL(c.GoogleRedirectURL); err != nil { return fmt.Errorf("GOOGLE_REDIRECT_URL: %w", err) }
		if err := validateHTTPSURL(c.PublicURL); err != nil { return fmt.Errorf("SERVER_URL: %w", err) }
		switch storageDriver {
		case "r2": return errors.New("R2 is not configured in this deployment; use STORAGE_DRIVER=b2 for Backblaze B2")
		case "b2":
			if err := validateAbsoluteURL(c.B2Endpoint); err != nil { return fmt.Errorf("B2_ENDPOINT: %w", err) }
			if c.B2Bucket == "" || c.B2KeyID == "" || c.B2ApplicationKey == "" || c.B2Region == "" { return errors.New("Backblaze B2 storage configuration must be set in production") }
		}
	}
	return nil
}

func normalizeFrontendURL(value string) string { value = strings.TrimSpace(value); if value == "" { return "" }; return strings.TrimRight(value, "/") }
func normalizeEndpoint(value, region string) string {
	value = strings.Trim(strings.TrimSpace(value), "\"'"); value = strings.TrimRight(value, "/"); region = strings.Trim(strings.TrimSpace(region), "\"'")
	if value != "" { if parsed, err := url.Parse(value); err == nil && parsed.Host != "" && (parsed.Scheme == "http" || parsed.Scheme == "https") { return value }; if parsed, err := url.Parse("https://" + value); err == nil && parsed.Host != "" && parsed.Path == "" { return "https://" + value } }
	if region != "" { return "https://s3." + region + ".backblazeb2.com" }
	return ""
}
func validateFrontendURL(value string) error {
	if err := validateAbsoluteURL(value); err != nil { return err }
	parsed, _ := url.Parse(strings.TrimSpace(value)); if parsed.Path != "" && parsed.Path != "/" { return errors.New("must not include a URL path") }; if parsed.RawQuery != "" || parsed.Fragment != "" { return errors.New("must not include query or fragment components") }; return nil
}
func validateAbsoluteURL(value string) error {
	parsed, err := url.Parse(strings.TrimSpace(value)); if err != nil || parsed.Host == "" || (parsed.Scheme != "http" && parsed.Scheme != "https") { return errors.New("must be an absolute http or https URL") }; if parsed.User != nil { return errors.New("must not include user information") }; return nil
}
func validateHTTPSURL(value string) error {
	if err := validateAbsoluteURL(value); err != nil { return err }
	parsed, _ := url.Parse(strings.TrimSpace(value)); if !strings.EqualFold(parsed.Scheme, "https") { return errors.New("must use HTTPS in production") }; return nil
}
