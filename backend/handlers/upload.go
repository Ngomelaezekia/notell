...
func (h *UploadHandler) cleanupUnclaimedUploads() {
	...
	for _, upload := range uploads {
		// Delete only while the row is still unclaimed. This makes cleanup
		// atomic with respect to a concurrent CreatePost claim and prevents
		// stale-upload cleanup from deleting a newly claimed upload.
		...
		if h.Storage != nil {
			if err := h.Storage.Delete(context.Background(), key); err != nil {
				// Continue local cleanup even if durable-storage deletion is
				// temporarily unavailable. Reconciliation will remove the
				// now-unreferenced durable object later.
			}
		}
		if claimed.Path != "" {
			if err := os.Remove(claimed.Path); err != nil && !os.IsNotExist(err) {
				// Local cleanup is best-effort; the durable DB record is already
				// gone and the staging path is not exposed to clients.
			}
		}
	}
}
