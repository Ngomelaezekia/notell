...
func (h *UploadHandler) UploadMedia(c *gin.Context) {
	...
	if err := mediaservice.ValidateUploadMetadata(contentType, file.Size, file.Filename); err != nil { c.JSON(http.StatusBadRequest, gin.H{"message": err.Error()}); return }
	...
	if err := h.DB.Transaction(func(tx *gorm.DB) error {
		...
		_ = h.Storage.Delete(context.Background(), key); _ = os.Remove(filePath)
		c.JSON(http.StatusInternalServerError, gin.H{"message": "failed recording uploaded file"}); return
	}
