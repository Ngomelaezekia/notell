...
func (h *AuthHandler) setSessionCookie(c *gin.Context, name, value string, maxAge int) {
	...
	domain := ""
	if h.Config.AppEnv == "production" && strings.HasSuffix(h.Config.FrontendURL, ".onrender.com") {
		domain = ".onrender.com"
	}
	c.SetCookie(name, value, maxAge, "/", domain, h.Config.AppEnv == "production", true)
}
...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if input.Username == "" {
		c.JSON(400, gin.H{"message": "username cannot be empty"})
		return
	}
	if len([]rune(input.Country)) > 100 || len([]rune(input.City)) > 100 || len([]rune(input.Bio)) > 2000 {
		c.JSON(400, gin.H{"message": "profile field is too long"})
		return
	}
	hash, err := bcrypt.GenerateFromPassword([]byte(input.Password), bcrypt.DefaultCost)
	if err != nil {
		c.JSON(500, gin.H{"message": "failed hashing password"})
		return
	}
	hashString := string(hash)
	user := models.User{Username: input.Username, Email: input.Email, PasswordHash: &hashString}
	if input.Country != "" {
	...
	if err := h.createSession(c, user); err != nil {
		c.JSON(500, gin.H{"message": "failed creating session"})
		return
	}
...
func (h *AuthHandler) Me(c *gin.Context) {
	...
	var user models.User
	err := h.DB.Select("id", "username", "email", "profile_picture", "cover_picture", "bio", "city", "country", "status", "allow_followers", "created_at", "updated_at").First(&user, userID).Error
	if err != nil {
		if errors.Is(err, gorm.ErrRecordNotFound) {
			c.JSON(401, gin.H{"message": "session user not found"})
			return
		}
		c.JSON(500, gin.H{"message": "failed to load current user"})
		return
	}
...
func (h *AuthHandler) GoogleLogin(c *gin.Context) {
	...
	if err != nil {
		c.JSON(500, gin.H{"message": "failed to start Google authentication"})
		return
	}
	h.setSessionCookie(c, "oauth_state", state, 300)
	h.setSessionCookie(c, "oauth_mode", mode, 300)
	c.Redirect(307, h.googleOAuthConfig().AuthCodeURL(state, oauth2.AccessTypeOffline))
}
...
func (h *AuthHandler) getGoogleUser(ctx context.Context, token *oauth2.Token) (*GoogleUser, error) {
	...
	if googleUser.ID == "" || googleUser.Email == "" || !googleUser.VerifiedEmail {
		return nil, errors.New("google account is missing required verified identity")
	}
...
func (h *AuthHandler) createGoogleUser(googleUser *GoogleUser) (*models.User, error) {
	...
	for attempt := 0; attempt < 5; attempt++ {
		...
		user := models.User{Username: username, Email: strings.ToLower(googleUser.Email), GoogleID: &googleUser.ID, ProfilePicture: &profilePicture}
		if err := h.DB.Create(&user).Error; err != nil {
			continue
		}
		return &user, nil
...
func (h *AuthHandler) GoogleCallback(c *gin.Context) {
	...
	if err != nil || cookieState == "" || cookieState != c.Query("state") {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=invalid_state")
		return
	}
	...
	h.setSessionCookie(c, "oauth_mode", "", -1)
	code := c.Query("code")
	if code == "" {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=no_code")
		return
	}
	ctx := c.Request.Context()
	oauthToken, err := h.googleOAuthConfig().Exchange(ctx, code)
	if err != nil {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=exchange_failed")
		return
	}
	googleUser, err := h.getGoogleUser(ctx, oauthToken)
	if err != nil {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=user_fetch_failed")
		return
	}
	existingUser, findErr := h.findGoogleUser(googleUser)
	if mode == "login" {
		if findErr != nil {
			if errors.Is(findErr, gorm.ErrRecordNotFound) {
				c.Redirect(307, h.Config.FrontendURL+"/auth?error=google_account_not_registered")
				return
			}
			c.Redirect(307, h.Config.FrontendURL+"/auth?error=google_login_failed")
			return
		}
		if err := h.createSession(c, *existingUser); err != nil {
			c.Redirect(307, h.Config.FrontendURL+"/auth?error=session_failed")
			return
		}
		c.Redirect(307, h.Config.FrontendURL+"/")
		return
	}
	if findErr == nil {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=google_account_already_registered")
		return
	}
	if !errors.Is(findErr, gorm.ErrRecordNotFound) {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=google_signup_failed")
		return
	}
	user, err := h.createGoogleUser(googleUser)
	if err != nil {
		c.Redirect(307, h.Config.FrontendURL+"/auth?error=google_signup_failed")
		return
	}
	if err := h.createSession(c, *user); err != nil {
		c.Redirect(307, h.Config.FrontURL+"/auth?error=session_failed")
		return
	}
	c.Redirect(307, h.Config.FrontendURL+"/")
}
