...
func (h *AuthHandler) Register(c *gin.Context) {
	...
	if err != nil { c.JSON(http.StatusInternalServerError, gin.H{"message": "failed hashing password"}); return }
	hashString := string(hash)
	user := models.User{Username: input.Username, Email: input.Email, PasswordHash: &hashString, Country: input.Country, City: input.City, Bio: input.Bio}
	if err := h.DB.Create(&user).Error; err != nil { c.JSON(http.StatusConflict, gin.H{"message": "username or email already exists"}); return }
	if err := h.createSession(c, user); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"message": "failed creating session"}); return }
...
func (h *AuthHandler) Login(c *gin.Context) {
	...
	if err := h.DB.Where("email = ?", input.Email).First(&user).Error; err != nil { c.JSON(http.StatusUnauthorized, gin.H{"message": "invalid credentials"}); return }
	if user.PasswordHash == nil || bcrypt.CompareHashAndPassword([]byte(*user.PasswordHash), []byte(input.Password)) != nil { c.JSON(http.StatusUnauthorized, gin.H{"message": "invalid credentials"}); return }
	if err := h.createSession(c, user); err != nil { c.JSON(http.StatusInternalServerError, gin.H{"message": "failed creating session"}); return }
...
func (h *AuthHandler) Me(c *gin.Context) {
	...
	if err != nil { c.JSON(http.StatusNotFound, gin.H{"message": "user not found"}); return }
...
func (h *AuthHandler) findOrCreateGoogleUser(googleUser *GoogleUser) (*models.User, error) {
	var user models.User
	result := h.DB.Where("google_id = ? OR email = ?", googleUser.ID, googleUser.Email).First(&user)
	if result.Error == nil {
		updates := map[string]interface{}{}
		if user.GoogleID == nil { updates["google_id"] = googleUser.ID }
		if user.ProfilePicture == nil && googleUser.Picture != "" { updates["profile_picture"] = googleUser.Picture }
		if len(updates) > 0 { if err := h.DB.Model(&user).Updates(updates).Error; err != nil { return nil, err } }
		return &user, nil
	}
	if result.Error != gorm.ErrRecordNotFound { return nil, result.Error }
	baseUsername := strings.ToLower(strings.ReplaceAll(googleUser.Name, " ", "_"))
...
func (h *AuthHandler) GoogleCallback(c *gin.Context) {
	...
	if err != nil { c.Redirect(http.StatusTemporaryRedirect, h.Config.FrontendURL+"/auth?error=exchange_failed"); return }
	googleUser, err := h.getGoogleUser(oauthToken)
	if err != nil { c.Redirect(http.StatusTemporaryRedirect, h.Config.FrontendURL+"/auth?error=user_fetch_failed"); return }
	user, err := h.findOrCreateGoogleUser(googleUser)
	if err != nil { c.Redirect(http.StatusTemporaryRedirect, h.Config.FrontendURL+"/auth?error=user_create_failed"); return }
	if err := h.createSession(c, *user); err != nil { c.Redirect(http.StatusTemporaryRedirect, h.Config.FrontendURL+"/auth?error=session_failed"); return }
