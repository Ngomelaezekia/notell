...
func serveMedia(storage services.MediaStorage, access func(context.Context, string, uint) (bool, bool, error)) gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			if err == mediaservice.ErrMediaAccessDenied {
...
func main() {
	...
	// Older deployments created uploads.post_id as a UNIQUE index. That shape
	// prevents a post from owning both its primary media and a PostMusic upload.
	// Drop the legacy index before AutoMigrate so the model's normal index can be
	// created consistently on existing and fresh databases.
	if result := db.Exec(`DROP INDEX IF EXISTS idx_uploads_post_id`); result.Error != nil {
		log.Fatalf("Failed migrating upload post index: %v", result.Error)
	}
	if err := db.AutoMigrate(&models.User{}, &models.Post{}, &models.Comment{}, &models.Like{}, &models.Relationship{}, &models.Channel{}, &models.Notification{}, &models.Upload{}, &models.MediaMetadata{}, &models.MediaJob{}, &models.PostView{}, &models.PostMusic{}); err != nil {
	...
	r.GET("/uploads/:filename", middleware.OptionalAuth(cfg.JWTSecret), serveMedia(mediaStorage, func(ctx context.Context, filename string, userID uint) (bool, bool, error) {
		...
		if err != nil {
			return false, policy.Private, err
		}
