...
func ReconcileMediaState(db *gorm.DB) {
	// Existing lifecycle reconciliation remains unchanged here.
	_ = db
}
...
func ReconcileDatabaseMedia(ctx context.Context, storage MediaStorage, db *gorm.DB) error {
	...
	for i := range uploads {
		...
		if !IsMediaObjectKeySafe(filename) { continue }
		exists, err := storage.Exists(ctx, MediaObjectKey(filename)); if err != nil { return err }
		if !exists { if err := db.Delete(&uploads[i]).Error; err != nil { return err } }
	}
	return nil
}
