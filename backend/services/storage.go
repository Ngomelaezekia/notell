...
func (s *localMediaStorage) Put(_ context.Context, key, src, _ string) error {
	...
	if err := os.MkdirAll(filepath.Dir(dst), 0755); err != nil {
		return fmt.Errorf("create local media directory: %w", err)
	}
	in, err := os.Open(src)
	if err != nil {
		return fmt.Errorf("open local media source: %w", err)
	}
	defer in.Close()
	...
	if _, err := io.Copy(tmp, in); err != nil {
		tmp.Close()
		return fmt.Errorf("copy local media: %w", err)
	}
	...
	if err := os.Rename(tmpPath, dst); err != nil {
		return fmt.Errorf("commit local media: %w", err)
	}
	return nil
}
...
func (s *s3MediaStorage) Open(ctx context.Context, key, rng string) (io.ReadCloser, string, int64, string, error) {
	...
	if rng != "" {
		if !validOpenEndedByteRange(rng) {
			return nil, "", 0, "", errors.New("invalid byte range")
		}
	...
	go func() {
		reconcile := func() {
			...
			if s, ok := storage.(*s3MediaStorage); ok {
				if err := s.reconcile(ctx, db); err != nil {
...
func ReconcileDatabaseMedia(ctx context.Context, storage MediaStorage, db *gorm.DB) error {
	...
	for _, upload := range uploads {
		...
		if upload.PostID == nil {
			message += "; upload must be replaced"
		}
		if err := db.Model(&models.MediaMetadata{}).
			Where("upload_id = ?", upload.ID).
			Updates(map[string]any{"status": "failed", "processing_error": message}).Error; err != nil {
	...
	for paginator.HasMorePages() {
		...
		for _, object := range page.Contents {
			...
			if !errors.Is(err, gorm.ErrRecordNotFound) {
				return fmt.Errorf("find upload for %q: %w", filename, err)
			}
			created := time.Now()
			...
			if err := s.Delete(ctx, key); err != nil {
				return fmt.Errorf("delete orphan media %q: %w", key, err)
			}
		}
	}
	return nil
