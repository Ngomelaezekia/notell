...
func syncConfiguredProviderRights(ctx context.Context) (int, error) {
	...
	if !ok || postgres == nil { return 0, errors.New("postgres rights store is required for provider synchronization") }
...
func upsertRightsRecord(ctx context.Context, store *postgresMusicRightsStore, record rightsSyncRecord) error {
	...
	if record.Provider == "" || record.ProviderTrackID == "" || record.Territory == "" { return errors.New("incomplete provider rights record") }
	if record.Status == "" { record.Status = "active" }
	if record.Source == "" { record.Source = "provider" }
...
func startProviderRightsSync() {
	...
	go func() {
		run := func() {
			...
			if err != nil { logProviderSyncError(err); return }
			if count > 0 { logProviderSyncSuccess(count) }
		}
		run()
...
func logProviderSyncError(err error) { fmt.Printf("music provider rights sync failed: %v\n", err) }
func logProviderSyncSuccess(count int) { fmt.Printf("music provider rights sync updated %d records\n", count) }
