...
func syncRightsFromJSON(ctx context.Context, body []byte) (int,error) {
	...
	runID,err:=beginRightsSyncRun(ctx,postgres.pool,providerOrFirstRecord(payload.Records,provider),mode,"provider-sync"); if err!=nil{return 0,err}; finishFailed:=true; defer func(){if finishFailed{finishRightsSyncRun(context.Background(),postgres.pool,runID,"failed",0,err)}}()
	...
	if err:=tx.Commit(ctx);err!=nil{return 0,err}; postgres.mu.Lock(); postgres.cache=make(map[string]cachedRights); postgres.mu.Unlock(); finishRightsSyncRun(context.Background(),postgres.pool,""+runID,"succeeded",len(payload.Records),nil); finishFailed=false; return len(payload.Records),nil
}
