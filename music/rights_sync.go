...
func syncRightsFromJSON(ctx context.Context, body []byte) (int,error) {
	...
	if err:=tx.Commit(ctx);err!=nil{return 0,err}; postgres.mu.Lock(); postgres.cache=make(map[string]cachedRights); postgres.mu.Unlock(); return len(payload.Records),nil
}
